
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
 * This is a barrel export file for all models and their related types.
 *
 * 🟢 You can import this file directly.
 */
export type * from './models/Workout'
export type * from './models/ExerciseAlternative'
export type * from './models/FormGuide'
export type * from './models/NigerianFood'
export type * from './models/FoodAlternative'
export type * from './models/MealPlan'
export type * from './models/Meal'
export type * from './models/MealPlanItem'
export type * from './models/WorkoutLog'
export type * from './models/User'
export type * from './models/Conversation'
export type * from './models/Message'
export type * from './commonInputTypes'